...
if player_ctrl == nil then
        player_ctrl = {
                ...
                mode = 0, -- modes are 0 (ghost)  1 (drive)   2 (on foot)
...
player_ctrl.speedo = get_hud_root():addChild("ShadowText",{font = "misc.fixed", charHeight = 13})
player_ctrl.speedo.resize = Hud.BOTTOM_LEFT()
player_ctrl.speedo.text = "Ghost Mode"
...
function player_ctrl:update (elapsed)
        ...
        if self.mode == 0 then
                ...
                -- update speedo
                do
                        local x,y,z = unpack(self.camFocus)
                        local bearing = self.camYaw
                        
                        local speed_units = active_speed*60*60/1000

                        self.speedo.text =
                        string.format("Pos: (%+5.0f, %+5.0f, %+0.0f)\nBearing: %0.2f\nSpeed: %d KM/H\n",
                                      x,y,z,
                                      bearing,
                                      speed_units
                                     )
        ...
        elseif self.mode == 1 then
                ...
                if vehicle.getSpeed ~= nil then
                        if self.speedo ~= nil then
                                ...
                                local speed_units = vehicle:getSpeed()*60*60/1000
                                self.speedo.text = string.format("Pos: (%+5.0f, %+5.0f, %+0.0f)\nBearing: %0.2f\nSpeed: %d KM/H\n",
                                                      x,y,z,
                                                      bearing,
                                                      speed_units
                                                     )
                        ...
                        if vehicle.fovScale then
                                ...
                                gfx_option("FOV", fovPlot[vehicle:getSpeed()])
                        end
                end
...
function player_ctrl:grab() -- TODO: rewrite this hacky part
...
function pick_pos_local()
    return pick_obj().instance.body:worldToLocal(pick_pos())
end
