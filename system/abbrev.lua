...
local function sky_material2(tab)
        local name = fqn(curried_name)
        tab = tab or {}
        if tab.emissiveMap then tab.emissiveMap = fqn_ex(tab.emissiveMap, name) end
...
local function material2(tab)
        ...
        local function qualify(t)
                for _,k in ipairs{"diffuseMap", "normalMap", "specularMap", "translucencyMap", "emissiveMap", "paintColour"} do
