...
local function configuration_defines ()
        local defines = " -DUSE_FOG="..(debug_cfg.fog and "1" or "0")
                      ...
                      .." -DGAMMA_CORRECTION_IN="..tostring(debug_cfg.gammaCorrectionIn)
                      .." -DGAMMA_CORRECTION_OUT="..tostring(debug_cfg.gammaCorrectionOut)
                      .." -DUSE_REVERSE_SPECULAR="..(debug_cfg.reverseSpecular and "1" or "0")
                      .." -DMAX_LIGHT_RANGE=".. debug_cfg.maxLightRange
...
local function shader_particle_names (...)
        local function shader_particle_names_aux (prefix, opacity, emissive)
                return string.format("%s:%s_%s",
                        prefix,
                        opacity==0 and "O" or opacity==1 and "A" or opacity==2 and "L" or "P",
                        emissive and "E" or "e"
                )
        end
...
local function make_particle_program_cg (opacity, emissive)
        local fname, vname = shader_particle_names(opacity, emissive)
        echo ("Compiling shader: ", vname, fname)
        local defines = "-O3 "
                     .. "-DOPACITY="..tostring(opacity).." "
        ...
        if not emissive then
                vp:setAutoConstantInt("scene_ambient_colour","AMBIENT_LIGHT_COLOUR")
                vp:setAutoConstantInt("sun_diffuse_colour", "LIGHT_DIFFUSE_COLOUR", 0)
                vp:setAutoConstantInt("misc", "SHADOW_COLOUR")
        end
...
function do_reset_particle_shaders ()
        for _,opacity in ipairs{0,1,2,3} do
                for _,emissive in ipairs{false, true} do
                        make_particle_program_cg(opacity, emissive)
                end -- emissive
        end -- opacity
end
...
local function make_program_emissive_cg (emissive_map, overlay, blended_bones, world)
        ...
        defines = defines .. " -DEMISSIVE_PART=1"
...
local function shader_names (...)
        local function shader_names_aux (prefix, dm, pma, em, nm, tm, spec_mode, paint_mode, blend, overlay, stipple, blended_bones, vcols, grass_lighting, world, rshadow, microflakes, forward_only)
                ...
                return string.format("%s:%s_%s_%s_%s_%s_%s_%01d_%s_%s_%01d_%01d_%s_%s_%s_%s_%s",
                        ...
                        pma and "P" or "p",
                        nm and "N" or "n",
                        spec_mode==4 and "G" or spec_mode==3 and "A" or spec_mode==2 and "D" or spec_mode==1 and "S" or "s",
                        tm and "T" or "t",
...
local function make_program_cg_ (category, diffuse_map, pma, emissive_map, normal_map, tran_map, spec_mode, paint_mode, blend, overlay, stipple, blended_bones, vcols, grass, world, rshadow, microflakes, forward_only)
        ...
        local has_spec_map = spec_mode==1 or spec_mode==4
        ...
        local defines = "-O3 -DUSE_VERTEX_COLOURS="..(debug_cfg.vertexDiffuse and tostring(vcols) or "0")
                      ...
                      .." -DUSE_SPECULAR_MAP="..(has_spec_map and "1" or "0")
                      .." -DUSE_SPECULAR_FROM_DIFFUSE="..(spec_mode==2 and "1" or "0")
                      .." -DUSE_SPECULAR_FROM_DIFFUSE_ALPHA="..(spec_mode==3 and "1" or "0")
                      .." -DUSE_GLOSS_FROM_SPECULAR_ALPHA="..(spec_mode==4 and "1" or "0")
        ...
        local tunit = 0
        if deferred_amb_sun or deferred_lights then tunit = 4 end -- account for the fat framebuffers
        local shadow_maps = ""
        local extra_maps = ""
        ...
        if forward then
                for i=0,blend-1 do
                        ...
                        if has_spec_map then 
                                extra_maps, tunit = sampler(extra_maps, tunit, "spec_map", i)
                                list_spec_maps = append_comma_always(list_spec_maps, "spec_map"..i)
                                extra_maps_again = extra_maps_again .. "sampler2D spec_map"..i..", "
                                extra_map_args = extra_map_args .. "spec_map"..i..", "
                        end
                        if tran_map then
                        ...
                        uniforms = uniforms .. "uniform float spec_diff_brightness"..i..","
                        uniform_args = append_comma_always(uniform_args, "spec_diff_brightness"..i)
                        list_spec_diff_brightnesses = append_comma_always(list_spec_diff_brightnesses, "spec_diff_brightness"..i)

                        uniforms = uniforms .. "uniform float spec_diff_contrast"..i..","
                        uniform_args = append_comma_always(uniform_args, "spec_diff_contrast"..i)
                        list_spec_diff_contrasts = append_comma_always(list_spec_diff_contrasts, "spec_diff_contrast"..i)
                ...
                defines = defines .. " -DSPEC_MAPS=\""..list_spec_maps.."\""
                ...
                defines = defines .. " -DSPEC_DIFF_CONTRASTS=\""..list_spec_diff_contrasts.."\""
                defines = defines .. " -DSPEC_DIFF_BRIGHTNESSES=\""..list_spec_diff_brightnesses.."\""
        end
        ...
        if forward_amb_sun or deferred_amb_sun then
                ...
                fp:setAutoConstantInt("scene_ambient_colour","AMBIENT_LIGHT_COLOUR")
                fp:setAutoConstantInt("misc", "SHADOW_COLOUR")
        end
        if forward_amb_sun then
        ...
        if forward then
                fp:setConstantFloat("alpha_rej", 0)
                if diffuse_map or normal_map or tran_map or has_spec_map then
                        fp:setAutoConstantFloat("time","TIME_0_X", 2310)
                        for i=0,blend-1 do
                ...
                for i=0,blend-1 do
                        if spec_mode == 2 then
                                fp:setConstantFloat("spec_diff_brightness"..i, 0)
                                fp:setConstantFloat("spec_diff_contrast"..i, 1)
                        end
                end
...
local function make_program_bloom_cg (horz)
        ...
        local defines = "-O3"
        defines = defines .. configuration_defines()
        defines = defines .. " -DHORZ="..(horz and "1" or "0")

        local vp = prog(vname,"cg","VERTEX")
        vp.profiles = {"vs_3_0", gl_profile_vert}
        vp.sourceFile = "system/bloom.cg"
        vp.compileArguments = defines
        vp.entryPoint = "vp_main"
        vp:reload()
        ...
        fp.compileArguments = defines
        fp.entryPoint = "fp_main"
        fp:setAutoConstantInt("texture_size", "PACKED_TEXTURE_SIZE", 0)
        fp:reload()
