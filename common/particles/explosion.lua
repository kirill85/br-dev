...
do
    local colour_base = vector3(1,1,1)
    local colour_curve = PlotV3 {
        [0]    = colour_base;
        [0.05] = colour_base;
        [0.15] = colour_base*vector3(1,0.8,0.8);
        [0.55] = colour_base*vector3(1,0.6,0.6);
        [1]    = colour_base*vector3(1,0.5,0.5);
    }
    ...
    particle "Explosion2" {
        map = "GenericParticleSheet.dds"; emissive = true; blending = "ADD";
        frames = particle_grid_frames(256,256); frame = 0;
        initialVolume = 10; maxVolume = 200; life = 2;
        behaviour = particle_behaviour_alpha_gas_ball;
        colourCurve = colour_curve;
    ...
    particle "Explosion3" {
        map = "GenericParticleSheet.dds"; emissive = true; blending = "OCCLUDE_ADD";
...
function explosion (pos, radius, force, num_flames)
    ...
    l.localPosition = pos + V_UP
    l.range = radius * 3
    for i=1,num_sprites1 do
        local offset = random_vector3_plane_z()
        local position = pos
        local velocity = 2*radius*offset + vector3(0,0,1);
    ...
    -- emit smoke cloud
    do
        local colour = 0.2*vector3(1,0.85,0.7)
        for i=1,1 do
            local dir = random_vector3_sphere()
            if dir.z < 0 then dir = dir * vector3(1,1,-1) end
            local dist = math.random()
            dist = dist * radius
            emit_textured_smoke(pos+dist*dir * vector3(1,1,0.5), dir*vector3(1,1,4), radius, radius*7, colour, 4)
        end
    end
